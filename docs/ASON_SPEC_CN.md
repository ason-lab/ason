...
**⚠️ 核心警告：复杂类型的 `@` 结构符不可省略！**

尽管终端基本数据（数字、字符串等）的 `@type` 是纯装饰并可彻底省去，但对于**复杂类型容器（嵌套对象、数组）**，即使不写具体基本类型，用以挂载层级关系的 `@` 后接 `{}` 或 `[]` **绝对不能省略**，它充当了极其关键的结构支架作用！
- ✅ **带注解嵌套**：`dept@{title@str}`
- ✅ **无注解纯结构嵌套**：`dept@{title}`（去掉了 `@str`，但必须保留 `@{}` 告知解析器进入下一个对象层级）
- ✅ **无注解数组**：`tags@[]`（保留 `@[]` 告知解析器这是一个数组边界）
